...
- GNU/LLVM derleme yolları freestanding C kodunu tutarlı biçimde `-Os` ile üretiyor; kernel düşük-bellek bootstrap stack sınırı değiştirilmeden başlangıçtaki 281 sektörden, AHCI okuma sürücüsü dahil 185 sektöre indi ve `0x90000` stack altında çalışma payı korundu.
- Erken boot aşamasında CPUID yetenek yoklaması eklendi; PAE, NX, TSC, RDRAND, MSR, APIC ve x2APIC durumu seri tanıda raporlanıyor. PAE destekleniyorsa sayfalama PAE backend’ine geçiyor; NX destekleniyorsa EFER.NXE ve donanımsal NX etkinleştiriliyor, desteklenmeyen CPU’larda legacy fallback korunuyor.
...
- Vektör duyarlı IDT/PIC/PIT, kuyruklu klavye sürücüsü ve IRQ dışında çalışan olay döngüsü eklendi. Doğrulanmış MADT bulunduğunda xAPIC/IOAPIC backend’i IRQ0, IRQ1 ve IRQ14’ü firmware interrupt override, polarite ve trigger bilgileriyle BSP’ye yönlendiriyor; geçiş başarısızsa tüm APIC durumu geri alınıp dual-8259 PIC yolu korunuyor.
...
- Q35/ICH9 için salt-okunur AHCI sürücüsü eklendi. PCI `01/06/01` sınıfı ve BAR5 doğrulanıyor; register alanı cache-disabled eşleniyor; destek varsa BIOS/OS sahiplik devri tamamlanıyor; PCI bus-master, descriptor’lar kurulmadan önce kapatılıyor. Tek slotlu IDENTIFY ve READ DMA/READ DMA EXT komutları 4 KiB bounce sayfasıyla seri ve bounded çalışıyor; hata/zaman aşımında DMA yetkisi fail-closed iptal ediliyor.
- 512 bayt sektör ve sürücüye özgü bounded istek sınırı olan blok aygıt katmanı eklendi. Başlatılmamış aygıtlar, null tamponlar, sıfır/taşan transferler ve aygıt sonunu aşan LBA aralıkları sürücü callback’i çağrılmadan reddediliyor; ATA ve AHCI’nin VFS’ye sunduğu görünüm bilerek salt-okunur tutuluyor.
- CMOS RTC duvar saati eklendi. Sürücü update-in-progress bitini bounded bekliyor, IRQ/NMI dışlama penceresinde iki eş örnek gerektiriyor, BCD/binary ve 12/24 saat biçimlerini çözüyor, geçersiz batarya/takvim değerlerini fail-closed reddediyor; `date` komutu doğrulanmış `YYYY-MM-DD HH:MM:SS RTC` çıktısı veriyor.
- Stage-2, ACPI RSDP imzasını EBDA ve BIOS ROM aralığında 16 bayt hizalı arıyor; çekirdek RSDP/RSDT/XSDT checksum ve boyutlarını sınırlandırarak doğruluyor. 64 KiB üstü tablolar, 256 üstü root girdileri, taşan adresler ve 32-bit çekirdeğin erişemediği XSDT adresleri tüketilmeden reddediliyor veya güvenle atlanıyor.
- MADT ayrıştırıcısı local APIC/x2APIC CPU kayıtlarını, IOAPIC MMIO adreslerini, GSI tabanlarını ve ISA interrupt override girdilerini bounded doğruluyor. xAPIC sürücüsü CPUID/MSR yeteneğini, IA32_APIC_BASE’i, firmware APIC kimliğini, IOAPIC sürüm/kimlik/redirection kapasitesini ve çakışmayan GSI aralıklarını donanım kayıtlarıyla eşleştiriyor; MMIO sayfaları cache-disabled eşleniyor ve route readback doğrulanıyor.
- HPET firmware tablosu ve canlı MMIO capability kaydı birbiriyle eşleştiriliyor; period alanı şartname sınırında doğrulanıyor, register sayfaları cache-disabled eşleniyor ve eski timer interrupt-enable durumları ana sayaç açılmadan temizleniyor. Sayaç nanosaniyeye taşma korumalı çevriliyor; 32-bit sayaç sarımı 100 Hz PIT bakım örneklemesiyle genişletiliyor. HPET yoksa scheduler kesmeleri ve çalışma süresi için mevcut PIT yolu korunuyor.
- HPET ve xAPIC birlikte kullanılabildiğinde local APIC’in 32-bit sayacı maskeli one-shot ölçümle 10 ms boyunca kalibre ediliyor, divide-by-16 ve periyodik mod register readback ile doğrulanıyor ve scheduler tick’i ayrı vektör 50 üzerinden üretiliyor. Kalibrasyon bounded süre içinde tamamlanmazsa veya sayaç/readback geçersizse IRQ0’ın IOAPIC/PIC üzerinden PIT’e dönmesi fail-closed korunuyor.
...
- Kullanıcı süreç yeniden-başlatma testi artık kırılgan sabit blok sayısına güvenmiyor; ilk dört eş tahsisin gerçek PMM maliyetini ölçüyor, maliyetlerin tutarlılığını ve her reap/restart döngüsünün aynı miktarı geri kazanıp yeniden tükettiğini doğruluyor.
...
- `scripts/reproducible-build-self-test.ps1` AHCI dahil optimize edilmiş 185 sektörlük imajı iki kez üretti ve iki SHA-256 özetinin byte düzeyinde aynı olduğunu doğruladı.
...
- `scripts/rtc-self-test.ps1` BCD/binary, 12/24 saat, AM/PM, century-byte fallback, leap-year ve bozuk tarih reddini doğruluyor. QEMU sabit `2024-02-29 23:58` fixture’ı gerçek CMOS port yolundan doğru çözüldü.
- `scripts/acpi-self-test.ps1` RSDP/RSDT/XSDT/HPET imza, checksum, uzunluk, reserved alan, 4 GiB üstü adres ve GAS varyantlarını host tarafında doğruluyor. `scripts/hpet-self-test.ps1` nanosaniye dönüşümünü, taşma doygunluğunu, monotonluğu ve 32-bit sayaç sarım genişletmesini doğruluyor; aynı test Linux CI’da ASan/UBSan altında çalışıyor.
- `scripts/madt-self-test.ps1` MADT checksum/uzunluk, local APIC kimlik bitmap’i, IOAPIC, override flag’leri, x2APIC, adres override, duplicate ve bozuk alt-yapı girdilerini host tarafında doğruluyor; aynı parser Linux CI’da ASan/UBSan altında çalışıyor.
- QEMU `-machine pc,hpet=on` koşusu canlı HPET MMIO sayacını ve periyodik local APIC scheduler timer’ını PAE/legacy sayfalama yollarında doğruluyor. `-machine pc,hpet=off` APIC açıkken IRQ0/PIT’in IOAPIC üzerinden çalıştığını; `-machine pc,acpi=off` ve `-cpu qemu32,apic=off` ise PIT + PIC fallback ile tam boot akışının sürdüğünü doğruluyor.
- QEMU `-machine q35,hpet=on` koşusu daha yeni ICH9 platformunda PCI keşfi, ACPI/MADT, IOAPIC yönlendirmesi, HPET, local APIC scheduler timer’ı ve tekrarlı kullanıcı süreç yaşam döngüsünü disksiz 54 kritik işaretçiyle tamamlıyor. Intel AHCI 1.3’te tanımlanan `01/06/01` PCI sınıfı ve BAR5 sözleşmesi Q35 ICH9 denetleyicisinde `controllers=1 usable-mmio=1` olarak doğrulandı; bozuk sınıf/header/BAR varyantları ayrı host testinde reddediliyor. 4 MiB SATA fixture ile `8192` sektörlük IDENTIFY, slot-0 DMA okuması, salt-okunur blok aygıt sözleşmesi, FAT mount ve `HELLO.TXT` okuması dahil 58 işaretçi geçiyor. `-Q35 -DisableHpet` diskli PIT/IOAPIC profili 57 işaretçiyle, `-Q35 -RequireHpet -Cpu qemu64` diskli PAE/NX profili 58 işaretçiyle doğrulandı.
...
- QEMU smoke testi başarılı; varsayılan profilde ACPI/MADT/APIC/RTC ve scheduler timer seçimi dahil 51 kritik boot/runtime işaretçisi, zorunlu HPET+local-APIC-timer profilinde 54 işaretçi ve deterministik ATA/FAT+HPET fixture’ında IRQ14/DMA/APIC bütünlük kanıtı dahil 60 işaretçi doğrulanıyor. HPET kapalı profil 53, ACPI kapalı profil 51, APIC kapalı profil 52 işaretçiyle fallback yollarını tamamlıyor.
...
- QEMU `-cpu qemu32,pae=off` koşusu legacy sayfalama fallback’ini (`paging mode=legacy hardware-nx=0x00000000`), canlı HPET’i, xAPIC yönlendirmesini ve tekrarlı kullanıcı süreç yaşam döngüsünü başarıyla tamamlıyor.
...
- Deterministik 4 MiB FAT16 imajıyla QEMU blok-aygıt/ATA/FAT/journal uçtan uca testi başarılı; ACPI/MADT/xAPIC/HPET/local APIC timer, RTC, IRQ14, bus-master DMA/PIO veri eşitliği, mount, kök dizin, dosya okuması ve persistent replay dahil 60 işaretçi doğrulanıyor.
...
- Kernel C kaynaklarının Clang static analyzer taraması, tasarım gereği kullanılan sabit MMIO adres denetleyicisi hariç eyleme dönük uyarı vermedi. Linux CI bu bilinen denetleyiciyi dışlayıp kalan analizleri ve ASan/UBSan kapılarını çalıştırıyor.
...
ATA IDENTIFY, QEMU i440FX/PIIX IDE PIO/IRQ14 ve PCI bus-master DMA okumaları doğrulandı; 48-bit destekli aygıtlarda yüksek LBA istekleri için EXT komutları hazır, 32-bit LBA API sınırı üzerindeki kapasite fail-closed reddediliyor. Q35/ICH9 AHCI yolu IDENTIFY, 1–8 sektörlük READ DMA/READ DMA EXT ve salt-okunur FAT erişimini destekliyor; şimdilik yalnız ilk uygun denetleyicideki ilk doğrudan SATA disk, polling ve tek komut slotuyla kullanılıyor. AHCI interrupt/MSI, NCQ, hot-plug, ATAPI, port reset/hata kurtarma ve yazma desteği henüz yok; 32-bit blok API’sini aşan diskler reddediliyor. Legacy ATA DMA okumaları 4 KiB/8 sektörlük parçalara ayrılıyor; herhangi bir controller/drive hatası motoru durduruyor, PCI bus-master bitini kapatıyor, DMA sayfalarını bırakıyor ve bounded PIO retry’a dönüyor. Gerçek donanım matrisi ve DMA yazma yolu henüz yoktur; journal yazmaları güvenlik amacıyla legacy ATA PIO’da kalıyor ve yalnız doğrulanmış journal window ile sınırlı. FAT metadata yazımı hâlâ kapalı. RTC yalnız duvar saati okur; timezone, saat ayarlama ve NTP yoktur. HPET yüksek çözünürlüklü monoton okuma ve local APIC timer kalibrasyonu sağlar; yokluğunda scheduler PIT’e döner. xAPIC/IOAPIC kesme yönlendirmesi IRQ0/1/14 ve local timer için tek BSP üzerinde çalışır; x2APIC, SMP başlatma, timer drift düzeltmesi ve gerçek donanım matrisi henüz yoktur. IPC bounded beklemeli receive, generation-PID hedefleme ve scheduler uyandırma desteğine sahip; en fazla sekiz bounded kullanıcı süreci destekleniyor. Authentication, secure boot, imzalı modüller, tam ASLR, ağ/USB/SMP ve tam VFS sonraki aşamalardır.

## Öncelikli sonraki geliştirmeler

1. **Depolama (P1):** ATA IRQ14/bus-master DMA ve Q35 AHCI salt-okunur yolları QEMU’da doğrulandı; sırada AHCI port reset/hata kurtarma, interrupt/MSI tamamlanması, çoklu aygıt ve gerçek donanım matrisi var. DMA yazma ve FAT metadata yazımını ancak bu hata kurtarma, journaling ve bütünlük kontrollerinden sonra aç.
2. **Donanım kapsamı (P2):** CMOS RTC, HPET monoton saat, xAPIC/IOAPIC yönlendirmesi ve kalibre edilmiş local APIC scheduler timer tamamlandı; sırada x2APIC/SMP, USB depolama/HID ve ağ sürücüleri var. Her biri için QEMU/host fixture ve gerçek donanım regresyonu yaz.
