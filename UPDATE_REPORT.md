...
- QEMU `-machine q35,hpet=on` koşusu daha yeni ICH9 platformunda PCI keşfi, ACPI/MADT, IOAPIC yönlendirmesi, HPET, local APIC scheduler timer’ı ve tekrarlı kullanıcı süreç yaşam döngüsünü 52 kritik işaretçiyle tamamlıyor. `-Q35 -DisableHpet` PIT/IOAPIC ve `-Q35 -RequireHpet -DisableApic` PIC/PIT geri dönüşleri de yerelde doğrulandı.
...
- QEMU smoke testi başarılı; varsayılan profilde ACPI/MADT/APIC/RTC ve scheduler timer seçimi dahil 49 kritik boot/runtime işaretçisi, zorunlu HPET+local-APIC-timer profilinde 52 işaretçi ve deterministik ATA/FAT+HPET fixture’ında IRQ14/DMA/APIC bütünlük kanıtı dahil 58 işaretçi doğrulanıyor. HPET kapalı profil 51, ACPI kapalı profil 49, APIC kapalı profil 50 işaretçiyle fallback yollarını tamamlıyor.
