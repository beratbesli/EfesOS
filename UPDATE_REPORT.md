...
- QEMU `-machine pc,hpet=on` koşusu canlı HPET MMIO sayacının ilerlediğini PAE ve legacy sayfalama yollarında doğruluyor; `-machine pc,acpi=off` koşusu ACPI/HPET yokluğunda, `-cpu qemu32,apic=off` koşusu APIC yeteneği yokluğunda PIT + PIC fallback ile tam boot akışının sürdüğünü doğruluyor.
...
- QEMU smoke testi başarılı; varsayılan profilde ACPI/MADT/APIC/RTC dahil 48 kritik boot/runtime işaretçisi, zorunlu HPET profilinde canlı sayaçla 50 işaretçi ve deterministik ATA/FAT+HPET fixture’ında IRQ14/DMA/APIC bütünlük kanıtı dahil 56 işaretçi doğrulanıyor. ACPI kapalı profil 47, APIC kapalı profil 48 işaretçiyle fallback yollarını tamamlıyor.
...
- Deterministik 4 MiB FAT16 imajıyla QEMU blok-aygıt/ATA/FAT/journal uçtan uca testi başarılı; ACPI/MADT/xAPIC/HPET, RTC, IRQ14, bus-master DMA/PIO veri eşitliği, mount, kök dizin, dosya okuması ve persistent replay dahil 56 işaretçi doğrulanıyor.
...
2. **Donanım kapsamı (P2):** CMOS RTC, HPET monoton saat ve temel xAPIC/IOAPIC yönlendirmesi tamamlandı; sırada PIT scheduler tick’ini kalibre edilmiş local APIC timer’a taşıma, ardından x2APIC/SMP, USB depolama/HID ve ağ sürücüleri var. Her biri için QEMU/host fixture ve gerçek donanım regresyonu yaz.
