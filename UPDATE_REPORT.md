...
- QEMU `-machine pc,hpet=on` koşusu canlı HPET MMIO sayacının ilerlediğini PAE ve legacy sayfalama yollarında doğruluyor; `-machine pc,acpi=off` koşusu ACPI/HPET yokluğunda PIT fallback ile tam boot akışının sürdüğünü doğruluyor.
...
- QEMU smoke testi başarılı; varsayılan profilde ACPI/RTC işaretleri dahil 46 kritik boot/runtime işaretçisi, zorunlu HPET profilinde canlı sayaçla 48 işaretçi ve deterministik ATA/FAT+HPET fixture’ında IRQ14/DMA bütünlük kanıtı dahil 54 işaretçi doğrulanıyor.
...
- Deterministik 4 MiB FAT16 imajıyla QEMU blok-aygıt/ATA/FAT/journal uçtan uca testi başarılı; ACPI/HPET, RTC, IRQ14, bus-master DMA/PIO veri eşitliği, mount, kök dizin, dosya okuması ve persistent replay dahil 54 işaretçi doğrulanıyor.
...
2. **Donanım kapsamı (P2):** CMOS RTC ve HPET monoton saat tamamlandı; yeni blok aygıt sözleşmesini kullanarak USB depolama/HID ve ağ sürücülerini, ardından APIC interrupt/timer routing’i ekle. Her biri için QEMU/host fixture ve gerçek donanım regresyonu yaz.
