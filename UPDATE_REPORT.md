...
- GNU/LLVM derleme yolları freestanding C kodunu tutarlı biçimde `-Os` ile üretiyor; kernel düşük-bellek bootstrap stack sınırı değiştirilmeden 281 sektörden 177 sektöre indi ve `0x90000` stack altında yaklaşık 60 KiB çalışma payı korundu.
...
- `scripts/reproducible-build-self-test.ps1` optimize edilmiş 177 sektörlük imajı iki kez üretti ve iki SHA-256 özetinin byte düzeyinde aynı olduğunu doğruladı.
...
- QEMU `-machine q35,hpet=on` koşusu daha yeni ICH9 platformunda PCI keşfi, ACPI/MADT, IOAPIC yönlendirmesi, HPET, local APIC scheduler timer’ı ve tekrarlı kullanıcı süreç yaşam döngüsünü 53 kritik işaretçiyle tamamlıyor. Intel AHCI 1.3’te tanımlanan `01/06/01` PCI sınıfı ve BAR5 sözleşmesi Q35 ICH9 denetleyicisinde `controllers=1 usable-mmio=1` olarak doğrulandı; bozuk sınıf/header/BAR varyantları ayrı host testinde reddediliyor. `-Q35 -DisableHpet` PIT/IOAPIC ve `-Q35 -RequireHpet -DisableApic` PIC/PIT geri dönüşleri de yerelde doğrulandı.
...
- QEMU smoke testi başarılı; varsayılan profilde ACPI/MADT/APIC/RTC ve scheduler timer seçimi dahil 50 kritik boot/runtime işaretçisi, zorunlu HPET+local-APIC-timer profilinde 53 işaretçi ve deterministik ATA/FAT+HPET fixture’ında IRQ14/DMA/APIC bütünlük kanıtı dahil 59 işaretçi doğrulanıyor. HPET kapalı profil 52, ACPI kapalı profil 50, APIC kapalı profil 51 işaretçiyle fallback yollarını tamamlıyor.
...
- Deterministik 4 MiB FAT16 imajıyla QEMU blok-aygıt/ATA/FAT/journal uçtan uca testi başarılı; ACPI/MADT/xAPIC/HPET/local APIC timer, RTC, IRQ14, bus-master DMA/PIO veri eşitliği, mount, kök dizin, dosya okuması ve persistent replay dahil 58 işaretçi doğrulanıyor.
