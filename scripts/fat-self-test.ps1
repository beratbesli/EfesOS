...
$output = Join-Path $build 'fat-host-test.exe'
$arguments = @('-std=c11', '-Wall', '-Wextra', '-Werror', '-I', (Join-Path $root 'fs'),
    (Join-Path $root 'tests\fat_host_test.c'), (Join-Path $root 'fs\fat.c'), '-o', $output)
& $compilerPath @arguments
if ($LASTEXITCODE -ne 0) {
    throw 'FAT host testi derlenemedi.'
}
& $output
if ($LASTEXITCODE -ne 0) {
    throw 'FAT host testi basarisiz.'
}
