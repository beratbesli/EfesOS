...
$testSource = Join-Path $projectRoot 'tests\ahci_layout_host_test.c'
$layoutSource = Join-Path $projectRoot 'drivers\ahci_layout.c'
$testBinary = Join-Path $buildDirectory 'ahci-layout-host-test.exe'
...
if ($LASTEXITCODE -ne 0) {
    throw 'AHCI yerlesim host testi derlenemedi.'
}
& $testBinary
if ($LASTEXITCODE -ne 0) {
    throw 'AHCI yerlesim host testi basarisiz.'
}
