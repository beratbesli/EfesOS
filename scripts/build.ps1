...
Invoke-Tool -Path $gcc -Arguments @('-m32', '-std=c11', '-ffreestanding', '-fno-pic', '-fno-pie', '-fno-stack-protector', '-nostdlib', '-nostartfiles', '-nodefaultlibs', '-Wall', '-Wextra', '-I', $includeDirectory, '-I', (Join-Path $projectRoot 'cpu'), '-I', (Join-Path $projectRoot 'memory'), '-I', (Join-Path $projectRoot 'process'), '-c', $kernelMainSource, '-o', $kernelMainObject) -FailureMessage 'Kernel ana kodu derlenemedi.'
...
Invoke-Tool -Path $gcc -Arguments @('-m32', '-std=c11', '-ffreestanding', '-fno-pic', '-fno-pie', '-fno-stack-protector', '-nostdlib', '-nostartfiles', '-nodefaultlibs', '-Wall', '-Wextra', '-I', $includeDirectory, '-I', (Join-Path $projectRoot 'cpu'), '-c', $keyboardSource, '-o', $keyboardObject) -FailureMessage 'Klavye surucusu derlenemedi.'
...
Invoke-Tool -Path $ld -Arguments @('-m', 'elf_i386', '-T', $linkerScript, '-o', $kernelElf, $kernelEntryObject, $kernelMainObject, $vgaObject, $keyboardObject, $idtObject, $interruptObject, $pmmObject, $pagingObject, $schedulerObject) -FailureMessage 'Kernel baglanamadi.'
