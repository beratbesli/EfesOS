...
$ahciDeviceTableSource = Join-Path $projectRoot 'drivers\ahci_device_table.c'
$ahciIrqStateSource = Join-Path $projectRoot 'drivers\ahci_irq_state.c'
$ahciRecoveryStateSource = Join-Path $projectRoot 'drivers\ahci_recovery_state.c'
...
$ahciDeviceTableObject = Join-Path $buildDirectory 'ahci_device_table.o'
$ahciIrqStateObject = Join-Path $buildDirectory 'ahci_irq_state.o'
$ahciRecoveryStateObject = Join-Path $buildDirectory 'ahci_recovery_state.o'
...
Invoke-CCompile -Source $ahciDeviceTableSource -Output $ahciDeviceTableObject -Includes @($includeDirectory, (Join-Path $projectRoot 'cpu'), (Join-Path $projectRoot 'drivers')) -FailureMessage 'AHCI aygit tablosu derlenemedi.'
...
Invoke-CCompile -Source $ahciRecoveryStateSource -Output $ahciRecoveryStateObject -Includes @((Join-Path $projectRoot 'drivers')) -FailureMessage 'AHCI recovery durum makinesi derlenemedi.'
