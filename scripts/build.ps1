...
Invoke-CCompile -Source $userProcessSource -Output $userProcessObject -Includes @($includeDirectory, (Join-Path $projectRoot 'memory'), (Join-Path $projectRoot 'process')) -FailureMessage 'Kullanici sureci derlenemedi.'
Invoke-CCompile -Source $elfLoaderSource -Output $elfLoaderObject -Includes @((Join-Path $projectRoot 'process')) -FailureMessage 'ELF yukleyici derlenemedi.'
...
Invoke-Tool -Path $linker -Arguments @('-m', 'elf_i386', '-T', $linkerScript, '-o', $kernelElf, $kernelEntryObject, $kernelMainObject, $panicObject, $syscallObject, $languageObject, $splashObject, $vgaObject, $serialObject, $keyboardObject, $pciObject, $ataObject, $idtObject, $pitObject, $systemObject, $interruptObject, $pmmObject, $pagingObject, $heapObject, $schedulerObject, $userProcessObject, $elfLoaderObject, $userDemoObject, $programsObject, $ramfsObject, $fatObject, $vfsObject, $gamesObject, $shellObject) -FailureMessage 'Kernel baglanamadi.'
