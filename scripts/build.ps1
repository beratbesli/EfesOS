...
Invoke-Tool -Path $gcc -Arguments @('-m32', '-std=c11', '-ffreestanding', '-fno-pic', '-fno-pie', '-fno-stack-protector', '-nostdlib', '-nostartfiles', '-nodefaultlibs', '-Wall', '-Wextra', '-I', $includeDirectory, '-I', (Join-Path $projectRoot 'shell'), '-c', $shellSource, '-o', $shellObject) -FailureMessage 'Shell derlenemedi.'
Invoke-Tool -Path $ld -Arguments @('-m', 'elf_i386', '-T', $linkerScript, '-o', $kernelElf, $kernelEntryObject, $kernelMainObject, $languageObject, $vgaObject, $keyboardObject, $idtObject, $interruptObject, $pmmObject, $pagingObject, $schedulerObject, $shellObject) -FailureMessage 'Kernel baglanamadi.'
