...
$output = Join-Path $build 'journal-host-test.exe'
$arguments = @('-std=c11', '-Wall', '-Wextra', '-Werror', '-Ifs',
    (Join-Path $root 'tests\journal_host_test.c'), (Join-Path $root 'fs\journal.c'), '-o', $output)
& $compilerPath @arguments
if ($LASTEXITCODE -ne 0) {
    throw 'Journal host testi derlenemedi.'
}
& $output
if ($LASTEXITCODE -ne 0) {
    throw 'Journal host testi basarisiz.'
}
