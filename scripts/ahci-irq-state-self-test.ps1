...
$testSource = Join-Path $projectRoot 'tests\ahci_irq_state_host_test.c'
$implementationSource = Join-Path $projectRoot 'drivers\ahci_irq_state.c'
$testBinary = Join-Path $buildDirectory 'ahci-irq-state-host-test.exe'
...
if ($LASTEXITCODE -ne 0) {
    throw 'AHCI IRQ durum makinesi host testi derlenemedi.'
}
& $testBinary
if ($LASTEXITCODE -ne 0) {
    throw 'AHCI IRQ durum makinesi host testi basarisiz.'
}
