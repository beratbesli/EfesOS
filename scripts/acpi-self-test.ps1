...
$testSource = Join-Path $projectRoot 'tests\acpi_host_test.c'
$parserSource = Join-Path $projectRoot 'drivers\acpi_tables.c'
$testBinary = Join-Path $buildDirectory 'acpi-host-test.exe'
...
if ($LASTEXITCODE -ne 0) {
    throw 'ACPI host testi derlenemedi.'
}
& $testBinary
if ($LASTEXITCODE -ne 0) {
    throw 'ACPI host testi basarisiz.'
}
