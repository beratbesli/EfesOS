...
6. Run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\reproducible-build-self-test.ps1` when changing the build or image pipeline.
7. Boot the result interactively in QEMU with the `-Run` switch for UI changes.
8. Keep C code freestanding and NASM code compatible with 16-bit BIOS or 32-bit protected mode as appropriate.
9. Do not commit `build/`, `tools/`, local IDE files or generated disk images.
10. Describe the QEMU and host tests performed in the pull request.
