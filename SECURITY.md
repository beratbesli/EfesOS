...
- User-frame mappings are claimed exactly once and released on unmap; alias attempts, freeing a still-mapped user frame, and user/kernel permission-boundary transitions fail closed.
...
- ELF executable pages carry a software execute bit, and scheduler plus syscall boundaries reject a ring-3 EIP that is not on a user executable page; user stacks also receive bounded per-boot layout diversification. On CPUs with PAE/NX, page tables additionally set the hardware NX bit for non-executable mappings; the legacy fallback still relies on software checks.
- Relocation-free position-independent `ET_DYN` images receive a bounded load bias selected from a non-overlapping user range; CPU RDRAND output is mixed into the layout seed when available, while a bounded PIT/address fallback remains for older CPUs. Dynamic-linker images are rejected because relocations are not implemented. This is partial ASLR, not a complete ASLR guarantee.
- Early boot probes CPUID and selects a PAE page-table backend when PAE is available; NXE and per-page hardware NX are enabled only when NX is also reported. Unsupported CPUs use the bounded legacy backend.
...
- ATA access is bounded and timeout-controlled, retries failed reads only after a bounded channel reset, and uses 48-bit commands when IDENTIFY advertises them. After IDT readiness, a legacy-primary PCI IDE controller is admitted for bus-master DMA only when its class, programming-interface bits and I/O BAR4 pass strict checks. DMA reads use PMM-owned, low identity-mapped 4 KiB PRDT/bounce pages that cannot cross a 64 KiB boundary; controller/drive errors revoke PCI bus-master permission, release the pages and fall back to PIO. IRQ14 has a bounded polling fallback, and requests are serialized so scheduler preemption cannot interleave transactions. The public 32-bit LBA API rejects capacities it cannot represent; FAT16 support is read-only and no shell command can write arbitrary disk sectors.
- The PIC starts with every hardware line masked. Only IRQ0, IRQ1 and IRQ14 have registered handlers and can be explicitly unmasked after their drivers are initialized; attempts to enable an unhandled line fail closed.
...
- The ATA raw-write path starts write-protected on every boot and is released only for a validated journal window after replay/sequence checks; unformatted media remains write-protected.
...
- ATA IRQ state transitions, errors, disabled delivery, polling fallback accounting and sequence wrap are host-tested. DMA tests cover PRDT alignment/64 KiB boundaries, controller admission, MWDMA selection and terminal status; CI attaches a QEMU PCI IDE disk and requires an 8-sector DMA read to match a PIO reference byte-for-byte with zero IRQ or DMA fallback.
- CMOS RTC reads wait for the update-in-progress window with a finite bound, sample twice under IRQ/NMI exclusion, require valid-battery status and reject unstable, malformed BCD/binary, invalid 12/24-hour, or impossible Gregorian dates. RTC absence disables wall-clock reporting instead of blocking boot; CI also boots a fixed leap-day clock fixture.
...
- User-task registration switches to the candidate CR3 under IRQ exclusion and requires an executable entry page plus a fully mapped writable initial stack; malformed internal contexts are rejected before a scheduler slot is allocated.
...
- Release tooling can produce and verify an external RSA-3072 signature over the complete image; CI checks both a valid signature and a mutated-image rejection. The key must be trusted out-of-band, and the BIOS floppy path does not itself enforce that signature.
