...
- Relocation-free position-independent `ET_DYN` images receive a bounded load bias selected from a non-overlapping user range; dynamic-linker images are rejected because relocations are not implemented. This is partial ASLR, not a complete ASLR guarantee.
